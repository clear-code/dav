...
export function syncCollection(options) {
  ...
  function transformResponse(xhr) {
    ...
    let responses = object.response.map(res => {
      ...
      if (!status) {
        for (const propstat of res.propstat) {
          if (!propstat.status)
            continue;
      ...
      return {
        ...
        statusText: status,
        props: getProps(res.propstat, { ignoreStatus: true })
      };
    });
...
function getProp(propstat, { ignoreStatus } = {}) {
  if (!ignoreStatus) {
 ...
 */
export function getProps(propstats, { ignoreStatus } = {}) {
  return mergeProps(
    propstats
      .map(propstat => getProp(propstat, { ignoreStatus }))
